    ...
    def AddInputAction(a, key_name):
        ...
        if len(a.OnBegin) > 0:
            ...
            if beginBehavior.Class.Name == "Behavior_ClientConsoleCommand":
                _controller_commands[key_name][beginBehavior.Command] = str.split(str(beginBehavior.Class),"\'")[1]
            else:
                # This is a special behaviour so store the class name for lookup instead of the command
                _controller_commands[key_name][beginBehavior.Name] = str.split(str(beginBehavior.Class),"\'")[1]
        
        if len(a.OnEnd) > 0:
            endBehavior = a.OnEnd[0]
            if endBehavior.Class.Name == "Behavior_ClientConsoleCommand":
                _controller_commands[key_name][endBehavior.Command] = str.split(str(endBehavior.Class),"\'")[1]
